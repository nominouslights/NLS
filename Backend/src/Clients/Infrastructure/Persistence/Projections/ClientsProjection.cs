...
/// aggregate name and the field mapping. (The Clients copy of Fleet's FleetProjection — each
/// module owns its base because IProjection is generic over the module's DbContext.)
///
...
/// </summary>
internal abstract class ClientsProjection<TSource, TRead> : IProjection<ClientsDbContext>
    ...
    where TRead : class, new()
{
    ...
    public async Task ApplyAsync(ClientsDbContext context, Guid aggregateId, CancellationToken cancellationToken)
    {
    ...
    private static Task<TRead?> FindRowAsync(ClientsDbContext context, Guid id, CancellationToken cancellationToken) =>
        context.Set<TRead>()
            .IgnoreQueryFilters()
            .FirstOrDefaultAsync(row => EF.Property<Guid>(row, "Id") == id, cancellationToken);

    private static Guid IdOf(ClientsDbContext context, TRead row) =>
