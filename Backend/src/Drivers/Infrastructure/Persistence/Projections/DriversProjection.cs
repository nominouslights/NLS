...
/// aggregate name and the field mapping. Module-local copy of Fleet's equivalent — modules
/// never reference each other, and Shared deliberately doesn't own this shape (projections
/// with fan-out, like <see cref="DriverProjection"/>, implement the interface directly).
///
...
/// </summary>
internal abstract class DriversProjection<TSource, TRead> : IProjection<DriversDbContext>
    ...
    where TRead : class, new()
{
    ...
    public async Task ApplyAsync(DriversDbContext context, Guid aggregateId, CancellationToken cancellationToken)
    {
    ...
    private static Task<TRead?> FindRowAsync(DriversDbContext context, Guid id, CancellationToken cancellationToken) =>
        context.Set<TRead>()
            .IgnoreQueryFilters()
            .FirstOrDefaultAsync(row => EF.Property<Guid>(row, "Id") == id, cancellationToken);

    private static Guid IdOf(DriversDbContext context, TRead row) =>
