...
/// aggregate name and the field mapping.
///
...
/// </summary>
internal abstract class FleetProjection<TSource, TRead> : IProjection<FleetDbContext>
    ...
    where TRead : class, new()
{
    ...
    /// <summary>
    /// Copies the aggregate's state onto the read row, including the id and any derived values.
    /// Derived values come from the aggregate's own computed properties, so C# is the single
    /// implementation — the previous design recomputed them in SQL and needed a bespoke
    /// <c>round_even</c> function plus a parity test to keep the two in step.
    /// </summary>
    protected abstract void Map(TSource source, TRead row);

    public async Task ApplyAsync(FleetDbContext context, Guid aggregateId, CancellationToken cancellationToken)
    {
    ...
    private static Task<TRead?> FindRowAsync(FleetDbContext context, Guid id, CancellationToken cancellationToken) =>
        context.Set<TRead>()
            .IgnoreQueryFilters()
            .FirstOrDefaultAsync(row => EF.Property<Guid>(row, "Id") == id, cancellationToken);

    private static Guid IdOf(FleetDbContext context, TRead row) =>
