...
/// aggregate name and the field mapping. (The Budgeting copy of Fleet's FleetProjection —
...
/// </summary>
internal abstract class BudgetingProjection<TSource, TRead> : IProjection<BudgetingDbContext>
    ...
    where TRead : class, new()
{
    ...
    public async Task ApplyAsync(BudgetingDbContext context, Guid aggregateId, CancellationToken cancellationToken)
    {
    ...
    public async Task RebuildAllAsync(BudgetingDbContext context, CancellationToken cancellationToken)
    {
    ...
    private static Task<TRead?> FindRowAsync(BudgetingDbContext context, Guid id, CancellationToken cancellationToken) =>
        context.Set<TRead>()
            .IgnoreQueryFilters()
            .FirstOrDefaultAsync(row => EF.Property<Guid>(row, "Id") == id, cancellationToken);

    private static Guid IdOf(BudgetingDbContext context, TRead row) =>
