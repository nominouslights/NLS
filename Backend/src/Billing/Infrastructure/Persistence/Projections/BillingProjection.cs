...
/// projects to exactly one read-model row carrying the same id (the Fleet base copied per
/// module — modules never share infrastructure classes). Subclasses supply only the aggregate
/// name and the field mapping.
///
...
/// </summary>
internal abstract class BillingProjection<TSource, TRead> : IProjection<BillingDbContext>
    where TSource : Entity
    where TRead : class, new()
{
    public abstract string AggregateType { get; }

    /// <summary>Copies the aggregate's state onto the read row, including the id and any derived
    /// values — derived values come from the aggregate's own computed properties, so C# is the
    /// single implementation.</summary>
    protected abstract void Map(TSource source, TRead row);

    public async Task ApplyAsync(BillingDbContext context, Guid aggregateId, CancellationToken cancellationToken)
    {
    ...
    public async Task RebuildAllAsync(BillingDbContext context, CancellationToken cancellationToken)
    {
    ...
    private static Task<TRead?> FindRowAsync(BillingDbContext context, Guid id, CancellationToken cancellationToken) =>
        context.Set<TRead>()
            .IgnoreQueryFilters()
            .FirstOrDefaultAsync(row => EF.Property<Guid>(row, "Id") == id, cancellationToken);

    private static Guid IdOf(BillingDbContext context, TRead row) =>
