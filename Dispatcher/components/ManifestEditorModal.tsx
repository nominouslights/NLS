...
export default function ManifestEditorModal({
  ...
  async function submit() {
    ...
    const input: TripManifestInput = {
      ...
      passengers: paxRowsToWire(passengers, stops),
  ...
  return (
    <ModalShell
      ...
      title={existing ? "Edit Manifest" : "Add Manifest"}
      onClose={onClose}
      error={error}
      maxWidth={880}
      footer={
    ...
    >
      ...
      >
        ...
        stops. Editing the manifest does not change trip status.
      </div>
      ...
      <div style={{ marginBottom: 16 }}>
        ...
        <div style={{ display: "flex", gap: 7, paddingTop: 4 }}>
          {(["Inbound", "Outbound"] as ManifestDirection[]).map((d) => (
            <OptChip key={d} active={direction === d} label={d} onClick={() => setDirection((c) => (c === d ? null : d))} />
          ))}
        </div>
      </div>
      ...
      <div style={{ marginBottom: 8 }}>
        <PassengerRowsEditor rows={passengers} stops={stops} onChange={setPassengers} />
      </div>
