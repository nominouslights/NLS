...
error handling across the whole workspace — `Backend/` (.NET 10, one class library per domain
composed by the API gateway) and `Dispatcher/` (Next.js 16 / React 19). You write and repair tests, harden error paths, and audit edge cases. You
...
- Tests live under `Backend/tests/`. Architecture tests (`tests/NorthernLink.ArchitectureTests`)
  enforce domain-library isolation (one library per domain, referencing only `NorthernLink.Shared`,
  layer boundaries as namespaces inside each assembly) — never weaken or delete them to make a
  build pass; if they fail, the design is wrong, report it.
- A test project references the domain library it tests (plus Shared transitively); there are no
  cross-domain test setups — cross-domain behavior is integration-events-only, same as production
  code.
- Local infra for integration tests: `aspire run` (from anywhere in the repo) or
  `dotnet run --project AppHost/NorthernLink.AppHost` — the AppHost lives at the workspace root,
  not inside `Backend/` (Postgres 17 on 5432, RabbitMQ on 5672/15672). Prefer real Postgres
  over mocks for anything touching schemas — an in-memory provider can't prove real SQL behavior.
  **Note**: local Postgres runs as a plain superuser by design, so Row-Level Security is
  unenforced there — superusers bypass RLS even with FORCE. Tenant-isolation/RLS tests need the
  non-superuser role from `Backend/docker/initdb/01-app-role.sql` applied by hand locally, or
  should run against the live server where that role is already provisioned.
...
- Cross-domain messaging (RabbitMQ integration events): test consumer behavior on malformed or
